...
describe('ExStudentsController (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    app = await createAuthenticatedE2EApp((builder) =>
      builder
        .overrideProvider(ExStudentsRepository)
        .useValue(exStudentsRepository),
    );
  });
  ...
  it('lists, creates, updates and deletes ex-students', async () => {
    ...
    expect(createResponse.body.data.exitReason).toBe('Mudança de cidade');
  ...
  it('filters and paginates ex-students list', async () => {
    exStudentsRepository.items.push(
      ...
      makeExStudent({
        ...
        category: 'Infantil 1',
        exitReason: 'Interesse por outro esporte',
