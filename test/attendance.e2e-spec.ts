...
describe('AttendanceController (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    attendanceRepository.classStudentIds.set(
      '22222222-2222-2222-2222-222222222222',
      ['11111111-1111-1111-1111-111111111111'],
    );
    attendanceRepository.classStudentIds.set('class-1', ['student-1']);
    attendanceRepository.classStudentIds.set('class-2', ['student-2']);
    attendanceRepository.validStudentClassLinks.add(
      '11111111-1111-1111-1111-111111111111:22222222-2222-2222-2222-222222222222',
    );
    attendanceRepository.studentNames.set(
      '11111111-1111-1111-1111-111111111111',
      'Ana Silva',
    );
  ...
  it('rejects attendance save when student is not linked to class', async () => {
    ...
    expect(response.body.message).toContain(
      'Students without active class link',
    );
  });
  ...
  it('filters and paginates attendance records list', async () => {
    ...
    const response = await request(app.getHttpServer())
      .get(
        '/api/attendance?page=1&perPage=1&search=carla&startDate=2026-04-01&endDate=2026-04-01&classId=class-2&studentId=student-2&status=late',
      )
      .expect(200);
  ...
  it('returns an operational summary for the selected attendance date', async () => {
    attendanceRepository.items.push(
      {
        id: 'att-0',
        studentId: '11111111-1111-1111-1111-111111111111',
        ...
        status: 'present',
        observations: '',
        savedAt: '2026-04-19T07:45:00.000Z',
      },
      {
        id: 'att-1',
        studentId: 'student-1',
    ...
    const response = await request(app.getHttpServer())
      .get('/api/attendance/summary?date=2026-04-19')
      .expect(200);

    expect(response.body.data.totalClasses).toBe(3);
    expect(response.body.data.completedClasses).toBe(3);
    expect(response.body.data.pendingClasses).toBe(0);
    expect(response.body.data.presentCount).toBe(2);
    expect(response.body.data.lateCount).toBe(1);
