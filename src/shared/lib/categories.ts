...
export const CATEGORY_GROUPS = [
  ...
  {
    label: 'Infantil',
    categories: ['Infantil 1', 'Infantil 2'] as SwimmingCategory[],
  },
  {
    label: 'Juvenil',
    categories: ['Juvenil 1', 'Juvenil 2'] as SwimmingCategory[],
  },
  {
    label: 'Junior',
    categories: ['Junior 1', 'Junior 2'] as SwimmingCategory[],
  },
...
const CATEGORY_GROUP_ORDER = TEACHER_CATEGORY_OPTIONS.reduce<
...
export function formatMixedCategoryLabel(categories: string[]): string {
  ...
  const indexes = sorted.map(
    (category) => CATEGORY_GROUP_ORDER[category] ?? -1,
  );
  const isContiguous = indexes.every(
    (index, current) => current === 0 || index === indexes[current - 1] + 1,
  );
