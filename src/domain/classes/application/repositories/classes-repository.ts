...
export interface ClassCategoryReference {
  id: string;
  name: string;
}

export interface CreateClassRepositoryInput extends CreateClassRequest {
  categoryIds: string[];
}
...
export type UpdateClassTeacherRoleRepositoryInput =
  UpdateClassTeacherRoleRequest;
...
export abstract class ClassesRepository {
  abstract list(
    params?: ListClassesRepositoryParams,
  ): Promise<PaginatedResult<ClassEntity>>;
  abstract listCategoryReferenceData(): Promise<ClassCategoryReference[]>;
  ...
  abstract transferTeacher(
    input: TransferTeacherRepositoryInput,
  ): Promise<ClassEntity>;
  abstract transferStudent(
