...
export class PoolsController {
  ...
  @Post()
  @Permissions('pools:write')
  async create(
    @Body(new ZodValidationPipe(createPoolSchema)) body: CreatePoolDto,
  ) {
    const { pool } = await this.createPool.execute(
      PoolRequestMapper.toCreate(body),
    );
  ...
  async update(
    ...
    @Body(new ZodValidationPipe(updatePoolSchema)) body: UpdatePoolDto,
  ) {
    const { pool } = await this.updatePool.execute(
      id,
      PoolRequestMapper.toUpdate(body),
    );
    return { data: PoolPresenter.toHTTP(pool) };
  }
