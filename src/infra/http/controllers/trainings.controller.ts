...
export class TrainingsController {
  ...
  async create(
    @Body(new ZodValidationPipe(createTrainingSchema)) body: CreateTrainingDto,
  ) {
    const { training } = await this.createTraining.execute(
      TrainingRequestMapper.toCreate(body),
    );
  ...
  async update(
    ...
    @Body(new ZodValidationPipe(updateTrainingSchema)) body: UpdateTrainingDto,
  ) {
    const { training } = await this.updateTraining.execute(
      id,
      TrainingRequestMapper.toUpdate(body),
    );
    return { data: TrainingPresenter.toHTTP(training) };
  }
