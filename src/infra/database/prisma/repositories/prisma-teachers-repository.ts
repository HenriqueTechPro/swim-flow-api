...
import {
  ...
  type ListTeachersRepositoryParams,
  type UpdateTeacherRepositoryInput,
...
import {
  PrismaTeacherMapper,
  type PrismaTeacherRecord,
} from '../mappers/prisma-teacher-mapper';

type TeacherSyncTx = {
  teacherCategory: {
    deleteMany(args: { where: { teacherId: string } }): Promise<unknown>;
    createMany(args: {
      data: Array<{ teacherId: string; categoryId: string }>;
    }): Promise<unknown>;
  };
  category: {
    findMany(args: {
  ...
  teacherCertification: {
    deleteMany(args: { where: { teacherId: string } }): Promise<unknown>;
    createMany(args: {
      data: Array<{ teacherId: string; name: string }>;
    }): Promise<unknown>;
  };
};
...
export class PrismaTeachersRepository implements TeachersRepository {
    ...
    return rememberPaginatedResult(
      ...
      async () => {
        ...
        const classStudentCountMap = await this.loadStudentCountsForClassIds(
          typedTeachers.flatMap((teacher) =>
            teacher.classTeachers.map((item) => item.classId),
          ),
        );

        return createPaginatedResult(
          typedTeachers.map((teacher) =>
            PrismaTeacherMapper.toDomain(teacher, classStudentCountMap),
          ),
          total,
          { page, perPage },
        );
  ...
  async create(input: CreateTeacherRepositoryInput) {
    ...
    return PrismaTeacherMapper.toDomain(
      teacher as PrismaTeacherRecord,
      new Map(),
    );
  }
  ...
  async update(id: string, input: UpdateTeacherRepositoryInput) {
    ...
    const classStudentCountMap = await this.loadStudentCountsForClassIds(
      (teacher as PrismaTeacherRecord).classTeachers.map(
        (item) => item.classId,
      ),
    );
  ...
  async remove(id: string) {
    ...
    const classStudentCountMap = await this.loadStudentCountsForClassIds(
      (existing as PrismaTeacherRecord).classTeachers.map(
        (item) => item.classId,
      ),
    );
    ...
    await this.cache.deleteMatching('teachers:list:');
    return PrismaTeacherMapper.toDomain(
      existing as PrismaTeacherRecord,
      classStudentCountMap,
    );
  }
  ...
  private async loadStudentCountsForClassIds(classIds: string[]) {
    ...
    return new Map<string, number>(
      studentCounts.map((item) => [item.classId, item._count.studentId]),
    );
  }
  ...
  private async syncCategories(
    ...
    const expandedCategories =
      expandTeacherCategorySelection(selectedCategories).map(
        parseCategoryValue,
      );
    ...
    const missing = expandedCategories.filter(
      (category) => !byName.has(category as never),
    );
    if (missing.length > 0) {
