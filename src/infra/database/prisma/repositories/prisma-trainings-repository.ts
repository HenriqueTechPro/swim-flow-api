...
import {
  PrismaTrainingMapper,
  type PrismaTrainingRecord,
} from '../mappers/prisma-training-mapper';
...
const normalizeTrainingLocation = (
  venueType: 'Piscina' | 'Mar' | 'Represa',
  locationName?: string,
) => (venueType === 'Piscina' ? '' : (locationName || '').trim());
...
export class PrismaTrainingsRepository implements TrainingsRepository {
    ...
    return rememberPaginatedResult(
      ...
      async () => {
        ...
        const where = {
          ...(type
            ? {
                type: TRAINING_TYPE_TO_PRISMA[
                  type as keyof typeof TRAINING_TYPE_TO_PRISMA
                ],
              }
            : {}),
        ...
        return createPaginatedResult(
          (trainings as unknown as PrismaTrainingRecord[]).map(
            PrismaTrainingMapper.toDomain,
          ),
          total,
          { page, perPage },
        );
  ...
  async create(input: CreateTrainingRepositoryInput) {
    const training = await this.prisma.training.create({
      data: {
        ...
        locationName: normalizeTrainingLocation(
          input.venueType,
          input.locationName,
        ),
        dayOfWeek: normalizeWeekday(input.dayOfWeek) ?? input.dayOfWeek,
    ...
    return PrismaTrainingMapper.toDomain(
      training as unknown as PrismaTrainingRecord,
    );
  }
  ...
  async remove(id: string) {
    ...
    await this.cache.deleteMatching('trainings:list:');
    return PrismaTrainingMapper.toDomain(
      existing as unknown as PrismaTrainingRecord,
    );
  }
}
